...
- Generated repository header artwork: `docs/assets/generate_header.py`
  ...
  domain surface (the spatial edge field reversal used by the README,
  the relaxed-state field profiles, and the F-Theta programme curve).
...
- Diagnostic and clock semantics model
  (`src/scpn_rfp_core/observability.py`), the second implemented
...
- Device configuration model (`src/scpn_rfp_core/`), the first implemented
...
- Machine-readable domain manifest `reactor-domain.json` binding the project
  ...
  (configuration `reversed_field_pinch`).
