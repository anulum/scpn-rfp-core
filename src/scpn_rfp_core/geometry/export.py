...
# SCPN RFP Core — open-format mesh exports

"""Binary STL, glTF 2.0 (GLB) and STEP exports of the device models.

The serialisers are the shared kernels of the pinned kernel library
(``scpn_reactor_kernels.geometry`` for STL/GLB,
``scpn_reactor_kernels.cad`` for STEP); this module owns only the
device-side provenance: the document ``extras`` of the GLB carry the model
schema, the source digests, the model digest, the segment count, the units
and the non-claims so a viewer can show where the meshes came from, and
``write_step`` writes exactly the digested bytes the CAD model record
carries. Coordinates are stored in metres; float32 storage is a format
constraint of STL and GLB and is stated in the contract document.
"""
...
from scpn_rfp_core.geometry.cad import DeviceModelCAD
from scpn_rfp_core.geometry.model import DeviceModel3D
...
def glb_extras(model: DeviceModel3D) -> dict[str, Any]:
    ...
    dict[str, Any]
        ...
        segment count, the units and the non-claims of the model record.
    """
...
def write_step(path: Path, model: DeviceModelCAD) -> int:
    ...
    normalised deterministic header.
