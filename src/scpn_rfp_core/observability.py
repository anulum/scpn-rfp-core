...
# SCPN RFP Core — diagnostic and clock semantics
...
identities for the reversed-field-pinch configuration this repository owns,
...
from scpn_rfp_core.configuration import ConsistencyFinding
...
RELAXATION_BAND_HZ: Final = (1.0e03, 5.0e05)

SHOT_DURATION_CEILING_S: Final = 1.0e00
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-RFP-CORE"
                )
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-RFP-CORE.
    acquisition_start_s
            ...
            if channel.candidate_id == "closed.resolved_mhd_mode" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            f"declared tearing/relaxation band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the RFP tearing and relaxation "
                            "fluctuation scale 1-500 kHz (Ortolani and Schnack "
                            "1993)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the RFP discharge scale of up to ~1 s "
                            "(Ortolani and Schnack 1993)"
                        ),
                    )
                )
