...
# SCPN RFP Core — device configuration container tests
...
def synthetic_configuration(
    identifier: str = "reversed_field_pinch",
...
def test_owned_identifier_constructs() -> None:
    """The owned identifier constructs with a consistent programme."""
    assert synthetic_configuration().identifier == "reversed_field_pinch"
...
def test_derived_pinch_parameter_formula() -> None:
    """The derived pinch parameter follows the Bennett-field formula."""
    derived = synthetic_configuration().derived_pinch_parameter()
    assert derived == pytest.approx((0.2 * 1.0 / 0.5) / 0.25)
...
def test_consistency_report_clean_and_findings() -> None:
    ...
    mismatch = synthetic_configuration(pinch_parameter=3.0)
    findings = mismatch.consistency_report()
    assert len(findings) == 1
    assert "deviates" in findings[0].message
    taylor = synthetic_configuration(pinch_parameter=1.0, plasma_current_ma=0.625)
    findings = taylor.consistency_report()
    assert len(findings) == 1
    assert "Taylor-relaxation" in findings[0].message
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: {**r, "fields": []}, "fields: must be an object"),
...
def test_from_record_field_type_violations() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["fields"]["pinch_parameter"] = True
    with pytest.raises(DeviceConfigurationError, match="pinch_parameter: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
...
def test_from_bytes_rejects_invalid_documents() -> None:
    ...
    record = synthetic_configuration().to_record()
    text = json.dumps(record).replace("2.0", "NaN", 1)
...
def test_integer_accepted_where_number_expected() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["limits"]["plasma_current_ma"] = 1
    restored = configuration_from_record(record)
    assert restored.limits.plasma_current_ma == 1.0
