...
# SCPN RFP Core — device geometry model tests
...
def test_non_numeric_fields_are_refused(value: object) -> None:
    ...
    record = reference_geometry().to_record()
    record["shell_thickness_m"] = value  # type: ignore[assignment]
    with pytest.raises(DeviceGeometryError, match="shell_thickness_m: must be"):
        geometry_from_record(record)
...
def test_invalid_json_and_non_finite_literals_are_refused() -> None:
    ...
    record = reference_geometry().to_record()
    text = json.dumps(record).replace("0.05", "NaN")
...
def test_geometry_is_frozen() -> None:
    ...
    with pytest.raises(dataclasses.FrozenInstanceError):
        reference_geometry().shell_thickness_m = 1.0  # type: ignore[misc]
