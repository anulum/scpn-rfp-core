...
the designated owner of device-level truth for the `reversed_field_pinch`
configuration of the SCPN Phase Orchestrator reactor registry
...
serialisation, and a data-only SPO registry pin (`src/scpn_rfp_core/`,
...
- the device boundary: plant and experiment truth, pulse lifecycle, and
  ...
  confining magnetic field and the edge toroidal field is reversed relative
  to the axis;
- diagnostic semantics (magnetics-heavy mode analysis, multiple-helicity
  versus quasi-single-helicity regime identification), reference frames,
  and clock identity declarations;
...
- **Axisymmetric tokamaks** (safety factor above unity, externally
  dominated toroidal field): `SCPN-TOKAMAK-CORE`.
- **Stellarator, heliotron, and torsatron** systems: `SCPN-STELLARATOR-CORE`.
- **Spheromak** (simply connected, no toroidal-field circuit):
  `SCPN-SPHEROMAK-CORE`.
- **Self-magnetic pinches** (linear Z-pinch class): `SCPN-Z-PINCH-CORE`.
- **Solver mathematics and validation evidence**: `SCPN-FUSION-CORE` remains
  the canonical owner of shared solver surfaces until an exact surface
  passes the reactor family migration gate. No solver code exists in, or
